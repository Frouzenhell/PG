...
public:
	...
	virtual void init()
	{
		...
		unsigned int tmpIndicates[6 * 4] = { 0, 3, 2, 1,
			4, 5, 6, 7,
			2, 3, 7, 6,
			0, 1, 5, 4,
			1, 2, 6, 5,
		...
		glGenBuffers(1, &tmpBuffer);
		initBuffer(tmpBuffer, tmpIndicates, 6 * 4 * sizeof(unsigned int), GL_ELEMENT_ARRAY_BUFFER, GL_STATIC_DRAW);
		m_buffers.push_back(tmpBuffer);
